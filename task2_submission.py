...
def classify_event(image):
    ...
    # Map predicted class index to event names
    event_names = [combat, rehab, military_vehicles, fire, destroyed_building]
    detected_event = event_names[predicted.item()]
...
def classification(img_name_list):
    for img_index in range(len(img_name_list)):
        img = "task2b/test/" + img_name_list[img_index] + ".jpeg"
        detected_event = classify_event(img)12
